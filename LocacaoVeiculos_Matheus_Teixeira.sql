...
create table LocacaoVeiculo (
	idLocacaoVeiculo int primary key auto_increment,
...
-- Erros --
describe Manutencao;
alter table Manutencao change column dataManuntecao dataManutencao date not null; 
################# implementação dos dados das tables ###########################
...
#### Alterei o "idLocacao" para "idLocacaoVeiculo" pois já há um "idLocacao" no codigo ####
insert into LocacaoVeiculo (idLocacaoVeiculo, idVeiculo) values
     (1, 2),
     (2, 3),
     (3, 5),
     (4, 8),
...
select valorTotal as total_arrecadado
from pagamento
where estado = 'pago';

select Veiculo.modelo, Veiculo.marca, COUNT(*) as numero_locacoes
from Veiculo
join LocacaoVeiculo on Veiculo.idVeiculo = LocacaoVeiculo.idVeiculo
group by Veiculo.idVeiculo
order by numero_locacoes desc;

select Cliente.nome, sum(Pagamento.ValorTotal) as devedor
from Cliente
